...
linkButtons.forEach(link => {
  link.addEventListener('click', function(){
    //console.log('click')
    linkButtons.forEach(btn => btn.classList.remove('on'))
    this.classList.add('on');
  })
})
// 사이트 제공 링크

let donerIntro = document.querySelectorAll('.doner-intro');
let donerLInk = document.querySelectorAll('.doner-link-box');
donerIntro.forEach(hover => {
...
donerLInk.forEach(out => {
  out.addEventListener('mouseleave', function(){
    //console.log('mouseleave')
    donerIntro.forEach(hover => hover.classList.remove('on'))
  })
})
